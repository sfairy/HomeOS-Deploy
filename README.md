...
- **四步向导**：SSH → Registry（ghcr）→ Deploy → Ops
...
1. **SSH**：填写 Host / Port / User / Password / Workdir → 测试连接  
2. **Registry**：填写 ghcr 用户名与 Token → 执行 `docker login`  
3. **Deploy**：确认工作目录后「开始部署」（会先检查 compose 文件是否存在）  
